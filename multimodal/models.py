...
class DualVisionEncoder(nn.Module):
    ...
    def __init__(self, vision : str):
        ...
        if vision == 'resnet50':
            ...
            self.embed_size = self.model_pa.fc.in_features # 2048
            #self.model_pa.fc = nn.Linear(self.embed_size, IMAGE_EMBEDDING_DIM)
            #self.model_lateral.fc = nn.Linear(self.embed_size, IMAGE_EMBEDDING_DIM)
            ...
            self.embed_size = self.model_pa.classifier.in_features # 1024
            #self.model_pa.classifier = nn.Linear(self.embed_size, IMAGE_EMBEDDING_DIM)
            #self.model_lateral.classifier = nn.Linear(self.embed_size, IMAGE_EMBEDDING_DIM)
...
class JointEncoder(nn.Module):
    ...
    def __init__(self, 
        ...
        if vision:
            ...
            self.dim_input += self.vision_encoder.embed_size * 2
